...
const COMMAND_INFO = {
	name: "uncard",
	description: "Cast a vote to remove someone's smart card"
}
...
async function execute(interaction) {
	...
	collector.on('end', async r => {
		print("Result: ", total)
		if (total >= 3) {
			await member.roles.remove(process.env["SMART_CARD_ID"])
			await msg.reply(`Get the fuck out, <@${member.id}>! 😊🎉`)
			print(`Killed ${user.username}!`)
		}
	})
}
